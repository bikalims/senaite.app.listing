...
class LuthandoMultiValue extends React.Component

  ###*
   * LuthandoMultiValue Field for the Listing Table
   *
   * A multi value field is identified by the column type "luthandomultivalue" in the
   * listing view, e.g.  `self.columns = {"Result": {"type": "luthandomultivalue"}, ... }`
   *
  ###
  constructor: (props) ->
    super(props)
  ...
  on_change: (event) ->
    ...
    console.debug "LuthandoMultiValue::on_change:name=#{name} value=#{values}"
  ...
  build_inputs: ->
    ...
    inputs = []
    for value in values
      console.log "LuthandoMultiValue::build_elements:value='#{value}'"
      inputs.push(
        <li>
          <input type="text"
  ...
  render: ->
    <div className={@props.field_css or "luthandomultivalue"}>
...
export default LuthandoMultiValue
